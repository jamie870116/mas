...
class AI2ThorEnv_cen(BaseEnv):
    ...
    def get_navigation_step(self, action: str, agent_id: int) -> List[str]:
        ...
        if not plan:
            ...
            if dist < 1.0 and obj_base_name in self.small_objects and not obj_base_name.startswith("Drawer"):
                if obj_id not in self.get_object_in_view(agent_id):
                    return [f"LookDown(30)"]
                
            if dist < 1.0 and obj_base_name.startswith("Drawer"):
                # TBD
                print(f"{obj_id} detected within 1 meter but no path found. Maybe blocked because other drawer is openned.")
                if self.save_logs:
                    self.logs.append(f"{obj_id} detected within 1 meter but no path found. Maybe blocked because other drawer is openned.")
                # if obj_id not in self.get_object_in_view(agent_id):
                #     return [f"LookDown(30)"]
                # else:
                return ["Idle"] 
        ...
        for aid in range(self.num_agents):
            ...
            if not success:
                if self.save_logs:
                    ...
                    if err:
                        self.logs.append(f'errorMessage: {err}')
                    else:
    ...
    def is_subtask_done(self, subtask: str, agent_id: int) -> bool:
        ...
        if name in self.look_actions:
            ...
            if dist > 1.0 and dist < 1.5 and obj_name in self.large_receptacles and obj_id in self.get_object_in_view(agent_id):
                ...
                if obj_id not in self.get_object_in_view(agent_id) and obj_name not in self.small_objects and not self.action_queue[agent_id]:
                    ...
                    print(f"Object {obj_id} is not in view of agent {agent_id} even though it is close (dist: {dist:.2f}m), may be block by other agent or need to adjust camera angle")
                    suc = False
                    return suc
        ...
        if suc:
            ...
            if name not in self.look_actions and name != "NavigateTo":
                # reset the pitch
                # print(f"Subtask {subtask} is successful, reset the camera pitch for agent {agent_id}")
